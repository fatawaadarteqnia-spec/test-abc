...
function extractJSONFromResponse(text: string): any {
  try {
    ...
    const plainJsonMatch = text.match(
      /(?:الرد:|response:|result:|\{)\s*(\{[\s\S]*?\})/i,
    );
    if (plainJsonMatch) {
      try {
...
async function tryDeepSeek(
  text: string,
  context?: string,
): Promise<CommandAnalysisResponse | null> {
...
function preprocessComplexText(
  text: string,
  context?: string,
): { processedText: string; analysis: any } {
  const analysis = {
...
function analyzeTextContext(text: string, existingContext?: string): string {
  ...
  if (
    text.includes("الكلمة") ||
    text.includes("النص") ||
    text.includes("الجملة")
  ) {
...
function fallbackAnalysis(text: string): CommandAnalysisResponse {
  ...
  if (
    cleanText.includes("امس��") ||
    cleanText.includes("احذف") ||
    cleanText.includes("إزالة") ||
    cleanText.includes("شيل")
  ) {
    // Try to find target
    ...
    const deleteIndex = words.findIndex(
      (word) =>
  ...
  if (
    cleanText.includes("أضف") ||
    cleanText.includes("ضع") ||
    cleanText.includes("اكتب") ||
    cleanText.includes("أريد") ||
    cleanText.includes("أحتاج")
  ) {
    ...
    if (cleanText.includes("في البداية") || cleanText.includes("في الأول")) {
      ...
      cleanText.includes("في النهاية") ||
      cleanText.includes("في الآخر")
    ) {
      position = "end";
      target = "end";
    ...
    if (!content) {
      ...
      const words = text.split(" ");
      const cmdIndex = words.findIndex(
        (word) =>
  ...
  if (
    ...
    cleanText.includes("خلاص") ||
    cleanText.includes("كفاية")
  ) {
    return {
      isCommand: true,
      commandType: "control",
...
export const analyzeCommandMultiAI: RequestHandler = async (req, res) => {
  try {
    ...
    console.log(
      `🤖 Available AI providers: ${availableProviders.map((p) => p.displayName).join(", ")}`,
    );
    ...
    for (const provider of availableProviders) {
      ...
      if (result) {
        ...
        if (result.confidence >= 0.6) {
          // Lower threshold for enhanced analysis
          console.log(
            `✅ نجح مع ${provider.displayName} (ثقة: ${result.confidence})`,
          );
...
export const testAPIKeyDirect: RequestHandler = async (req, res) => {
  try {
    ...
    try {
          ...
          try {
            ...
            const completion = await tempClient.chat.completions.create({
              ...
              messages: [
                {
                  role: "system",
                  content: 'أنت مساعد ذكي. رد بكلمة واحدة: "نجح"',
                },
              ...
              messages: [
                {
                  role: "system",
                  content:
                    'You are a helpful assistant. Reply with just one word: "success"',
                },
      ...
      if (result) {
        console.log(
          `API key test successful for ${providerConfig.displayName}`,
        );
        return res.json({
          success: true,
        ...
        console.log(
          `API key test failed - no result for ${providerConfig.displayName}`,
        );
        return res.json({
          success: false,
...
export const analyzeWithProvider: RequestHandler = async (req, res) => {
  try {
    ...
    fallbackResult.explanation =
      "خطأ في الذكاء الاصطناعي - تم استخدام التحليل الأساسي";
    res.json(fallbackResult);
  }
};
